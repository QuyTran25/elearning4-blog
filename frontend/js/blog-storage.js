...
async function initializeSampleBlogs() {
  ...
  if (storedVersion !== CURRENT_VERSION) {
    console.log('🔄 Updating to version ' + CURRENT_VERSION + ' (preserving all blogs)...')
  ...
  if (missingSampleBlogIds.length > 0) {
    console.log('📦 Loading ' + missingSampleBlogIds.length + ' missing sample blogs...')
    ...
    console.log('✅ Loaded 6 sample blogs + ' + userBlogs.length + ' user blogs (total: ' + mergedBlogs.length + ')')
  } else {
    console.log('✅ All blogs ready (' + blogs.length + ' total)')
  }
}
